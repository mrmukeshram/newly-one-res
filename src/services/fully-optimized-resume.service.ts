...
const validateInputs = (originalResume: string, jobDescription: string): void => {
  ...
  if (jobDescription.length > 5000) {
    throw new ResumeOptimizationError(
      "Job description is too long. Please shorten to under 5,000 characters",
      "JOB_DESC_TOO_LONG",
      false
    );
  }
...
export const buildOptimizedResume = async (
  ...
  for (let attempt = 0; attempt < finalConfig.maxRetries; attempt++) {
    try {
      ...
      // Success - check if content expansion is needed
      const contentAnalysis = analyzeContentLength(optimizedResume);
      
      if (contentAnalysis.expansionNeeded) {
        console.log(`Content too short (${contentAnalysis.wordCount} words), expanding...`);
        
        // Create expansion prompt with targeted sections
        const expansionPrompt = createContentExpansionPrompt(
          JSON.stringify(optimizedResume),
          jobDescriptionText,
          contentAnalysis.recommendedSections
        );
        ...
        try {
          ...
          if (validateResumeStructure(expandedResume)) {
            console.log(`Resume expansion successful - final length: ${analyzeContentLength(expandedResume).wordCount} words`);
            return expandedResume;
          }
        } catch (expansionError) {
          console.warn("Content expansion failed, returning original optimized version");
        }
      }

      console.log(`Resume optimization successful on attempt ${attempt + 1} - length: ${contentAnalysis.wordCount} words`);
      return optimizedResume;
