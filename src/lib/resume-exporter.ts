...
export const estimateResumeLayout = (resume: OptimizedResume): PageLayoutEstimation => {
  ...
  // Secondary Skills
  if (resume.secondarySkills && resume.secondarySkills.length > 0) {
    totalHeight += 400;
    totalHeight += estimateTextLines(resume.secondarySkills.join(', '), 90) * 200;
  }
...
export const autoReduceToSinglePage = (resume: OptimizedResume): OptimizedResume => {
  ...
  if (checkLayout.estimatedPageCount > 1.1) {
    ...
    // Remove secondary skills if still overflowing
    if (resume.secondarySkills?.length > 0) {
      const recheckLayout2 = estimateResumeLayout(condensedResume);
      if (recheckLayout2.estimatedPageCount > 1.1) {
        condensedResume.secondarySkills = [];
      }
    }
    ...
    const doc = new Document({
        sections: [{
            ...
            children: [
                ...
                // Additional Sections
                ...(resume.additionalSections?.filter(section => section && section.title && section.content && section.content.trim().length > 0).length > 0 ? [
                    new Paragraph({
                        border: { bottom: { color: '999999', space: 1, style: BorderStyle.SINGLE, size: 6 } },
                        spacing: { after: 60 }
                    }),
